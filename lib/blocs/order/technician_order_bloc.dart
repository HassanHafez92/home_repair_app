...
class TechnicianOrderBloc
  ...
  StreamSubscription<List<OrderModel>>? _incomingOrdersSubscription;
  ...
  Future<void> _onAcceptOrder(
    ...
    try {
      await _orderRepository.assignTechnicianToOrder(
        event.orderId,
        event.technicianId,
        event.estimate,
      );
  ...
  Future<void> _onRejectOrder(
    ...
    try {
      await _orderRepository.rejectOrder(event.orderId, event.reason);
    } catch (e) {
      emit(
        state.copyWith(
          status: TechnicianOrderStatus.failure,
